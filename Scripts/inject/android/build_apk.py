...
def patch_elf_hijack_needed(so_path: Path, victim_lib: str, inject_lib: str) -> bool:
    """Hijack an existing DT_NEEDED entry by overwriting its name in .dynstr.

    Instead of adding a new DT_NEEDED entry (requires spare DT_NULL + strtab
    padding), this overwrites the string of an existing dependency.  The inject
    library must call dlopen(victim_lib) in its constructor to restore the
    original dependency.

    Requirements: len(inject_lib) <= len(victim_lib)
    """
    print(f"  Hijacking DT_NEEDED: {victim_lib} → {inject_lib}")

    if len(inject_lib) > len(victim_lib):
        print(f"  ERROR: inject_lib ({len(inject_lib)} chars) longer than "
              f"victim_lib ({len(victim_lib)} chars)")
        return False
    ...
    for j in range(dyn_size // 16):
        ...
        if d_tag == DT_STRTAB:
            ...
            needed_entries.append((d_val, pos))
    ...
    # ---- Find the victim's string and overwrite ----
    victim_bytes = victim_lib.encode("ascii")
    inject_bytes = inject_lib.encode("ascii")

    for str_off, dyn_pos in needed_entries:
        name_start = strtab_foff + str_off
        name_end = data.index(0, name_start)
        name = data[name_start:name_end].decode("latin-1")
        ...
        if name == victim_lib:
            # Check if already patched
            if name == inject_lib:
                print(f"  Already hijacked ({victim_lib} → {inject_lib})")
                return True

            # Overwrite: write inject_lib name + null padding
            total_len = len(victim_bytes) + 1  # include original null terminator
            new_bytes = inject_bytes + b"\x00" * (total_len - len(inject_bytes))
            data[name_start:name_start + total_len] = new_bytes

            print(f"  Overwrote '{victim_lib}' ({len(victim_bytes)} chars) "
                  f"→ '{inject_lib}' ({len(inject_bytes)} chars) "
                  f"at strtab offset 0x{str_off:x}")

            # Write patched binary
            so_path.write_bytes(bytes(data))
            print(f"  Hijacked! {so_path.name} now loads {inject_lib} "
                  f"instead of {victim_lib}")
            return True

    print(f"  ERROR: '{victim_lib}' not found in DT_NEEDED entries")
    return False
...
def patch_arm64_split(
    ...
    # --- Step 1b: Replace anti-cheat .so files with NOOP stubs ---
    # SdkNgDetect (libunisec.so) checksums libGame.so .text section and kills
    # the process when it detects Dobby inline hooks. The NOOP stubs have all
    # exported functions patched to RET (return 0); JNI_OnLoad returns
    # JNI_VERSION_1_6.  Generated by analysis/patch_unisec_noop.py.
    anticheat_stubs = {
        "libdetect.so": OUTPUT_DIR / "libdetect_noop.so",
    }
    for ac_lib, stub_path in anticheat_stubs.items():
        ac_path = lib_dir / ac_lib
        if ac_path.exists() and stub_path.exists():
            shutil.copy2(stub_path, ac_path)
            print(f"  Replaced anti-cheat: {ac_lib} → {stub_path.name}")
    ...
    # --- Step 3: Hijack DT_NEEDED in libGame.so ---
    # libGame.so has 0 spare DT_NULL and 1 byte of .dynstr padding — cannot
    # add a new DT_NEEDED. Instead, overwrite existing "libOpenSLES.so" string
    # in .dynstr with "libinject.so\0\0\0". The linker loads libinject.so when
    # libGame.so loads.  Our JNI_OnLoad calls dlopen("libOpenSLES.so") to
    # restore the original audio library.
    print("\n[3/4] Hijacking DT_NEEDED in libGame.so...")
    game_so = lib_dir / "libGame.so"
    if not game_so.exists():
        raise FileNotFoundError(f"libGame.so not found in {lib_dir}")

    if not patch_elf_hijack_needed(game_so, "libOpenSLES.so", "libinject.so"):
        raise RuntimeError("Failed to hijack DT_NEEDED in libGame.so")
