...
# Anti-cheat .so files to replace with NOOP stubs in APK.
# SdkNgDetect checksums libGame.so .text and kills the process when it
# detects Dobby inline hooks.  The stubs have all exported functions
# patched to RET (return 0); JNI_OnLoad returns JNI_VERSION_1_6.
# Generated by: Scripts/inject/android/analysis/patch_unisec_noop.py
ANTICHEAT_REPLACE = {
    "lib/arm64-v8a/libdetect.so": f"{OUTPUT}/libdetect_noop.so",
}
            ...
            with zipfile.ZipFile(tmp_apk, 'w') as zout:
                for item in zin.infolist():
                    if item.filename == "lib/arm64-v8a/libinject.so":
                        continue  # skip old .so — will add updated one below
                    if item.filename in ANTICHEAT_REPLACE:
                        stub = ANTICHEAT_REPLACE[item.filename]
                        if os.path.exists(stub):
                            zout.write(stub, item.filename)
                            print(f"  Replaced anti-cheat: {item.filename} → {os.path.basename(stub)}")
                        else:
                            print(f"  WARNING: stub not found: {stub}, keeping original")
