-- Scripts/tests/test_pitchpot.lua
local T = dofile("C:\\temp\\Where Winds Meet\\Scripts\\tests\\run_test.lua")
T.reset()

local Reg = _G.Reg
local mod = Reg.module("actions.pitchpot")
...
T.run("state initialized with correct types", function()
	...
	T.assert_type(mod.state.hit_interval, "number", "hit_interval is number")
end)
...
-- 8. _start_polling and _stop_polling exist
...
-- 9. No hooks defined (this module uses timers, not hooks)
