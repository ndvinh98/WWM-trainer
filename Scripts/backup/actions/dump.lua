...
local function get_core()
	if not _core then
		...
		if not Constants then
			_log("ERROR: Constants is nil!")
			return nil
		end
		if not Constants.LIB_ROOT then
			_log("ERROR: Constants.LIB_ROOT is nil!")
			return nil
		end
		if not Utils then
			_log("ERROR: Utils is nil!")
			return nil
		end
		if not Utils.safe_dofile then
			_log("ERROR: Utils.safe_dofile is nil!")
			return nil
		end

		local path = Constants.LIB_ROOT .. "\\dump_core.lua"
		_log("Loading core from: " .. path)

		local ok, result = pcall(function()
			return Utils.safe_dofile(path, "DumpCore")
		end)

		if not ok then
			_log("ERROR: pcall failed: " .. tostring(result))
			return nil
		end

		_core = result
		if not _core then
			_log("ERROR: Failed to load DumpCore!")
		else
			_log("DumpCore loaded successfully")
		end
	end
	return _core
end
