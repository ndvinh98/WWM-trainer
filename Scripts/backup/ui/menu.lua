...
-- Load config (contains all tabs and action references)
local MenuConfig = Utils.safe_dofile(_SCRIPTS_ROOT .. "\\ui\\menu_config.lua", "MenuConfig")
Reg.set("MenuConfig", MenuConfig)

-- Load UI components
local UIInput = Utils.safe_dofile(_SCRIPTS_ROOT .. "\\ui\\components\\input.lua", "Input")
...
function Menu.create(scene)
	...
	_log("Initializing first tab...")
	pcall(function()
		tabBtns[1]:setTitleColor(Theme.to_c3b(Theme.COLORS.TAB_ACTIVE))
		Theme.apply_bg(tabBgs[1], Theme.BG.TAB_ACTIVE, 255)
	end)
	_log("Building toggles for tab 1...")
	local ok, err = pcall(function()
		buildToggles(1, false)
	end)
	if not ok then
		_log("ERROR building first tab toggles: " .. tostring(err))
	else
		_log("✓ First tab initialized with " .. #Menu.TABS[1].items .. " items")
	end
