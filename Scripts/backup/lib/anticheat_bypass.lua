...
local HookInterceptor = Reg.get("HookInterceptor")

local STATE_NAME = "ACB_STATE"
if not Reg.has(STATE_NAME) then
	Reg.set(STATE_NAME, {
		is_enabled = false,
		logging = true, -- toggle detailed intercept logging
...
local function register_orig_module(module_name)
	...
	local module = Utils.safe_import(module_name)
	if module then
...
local function make_replacement(spec, method_name)
	...
	return function(self_arg, ...)
		if _state.logging then
			local args = { ... }
			if #args > 0 then
				_log(prefix .. " args=" .. Utils.dump_value(args))
			else
				_log(prefix)
			end
		end
	...
	for _, spec in ipairs(specs) do
		...
		if spec.fields then
			for _, field in ipairs(spec.fields) do
				...
				rawset(target_module, field, Utils.create_empty_proxy())
			end
...
local function make_sa_log_interceptor()
	return function(self, kwargs)
		...
		if _state.logging then
			_log("[SA-Log BLOCKED] op=" .. operation .. " | " .. Utils.dump_value(kwargs))
		end
...
local function patch_drpf_config()
	local drpf_mod = Utils.safe_import("patch.sa_log_comp.drpf_config")
...
local function patch_sa_log_handler()
	local handler_mod = Utils.safe_import("patch.sa_log_handler")
	...
	rawset(SALogHandler, "log", function(self, operation, kwargs)
		if _state.logging then
			_log("[SALogHandler] log(" .. tostring(operation) .. ") kwargs=" .. Utils.dump_value(kwargs))
		end
	end)
	...
	rawset(SALogHandler, "get_dict", function(self, operation, kwargs)
		if _state.logging then
			_log("[SALogHandler] get_dict(" .. tostring(operation) .. ") kwargs=" .. Utils.dump_value(kwargs))
		end
		return {}
	end)
...
local function patch_qdata_handler()
	local qdata_mod = Utils.safe_import("hexm.client.net.qdata_log_handler")
		...
		for _, method in ipairs(methods_to_noop) do
			if type(rawget(QdataLogHandler, method)) == "function" then
				...
				rawset(QdataLogHandler, method, function(self, ...)
					if _state.logging then
						_log("[QData] " .. m .. " args=" .. Utils.dump_value({ ... }))
					end
				end)
			end
		end
...
local function patch_acsdk_sdk_manager()
	local sdk_manager_mod = Utils.safe_import("hexm.client.manager.sdk_manager")
...
local function run_verify_patches()
	...
	local base_log_mod = Utils.safe_import("patch.sa_log_comp.base_log")
	...
	local space_mod = Utils.safe_import("hexm.common.space_common")
	if space_mod then
	...
	local sdk_mgr_mod = Utils.safe_import("hexm.client.manager.sdk_manager")
	if sdk_mgr_mod then
	...
	local dmg_mod = Utils.safe_import("hexm.common.combat.anti_cheating.damage_stats_manager")
	if dmg_mod then
		...
		-- SA-log component in sdk_manager (report_sa_log, report_script_log, report_qdata_log)
		{
			...
			methods = {
				report_sa_log = function(self, operation, kwargs)
					if _state.logging then
						_log("[SDK] report_sa_log(" .. tostring(operation) .. ") kwargs=" .. Utils.dump_value(kwargs))
					end
				end,
				report_script_log = function(self, log_type, kwargs)
					if _state.logging then
						_log(
							"[SDK] report_script_log(" .. tostring(log_type) .. ") kwargs=" .. Utils.dump_value(kwargs)
						)
					end
				end,
				report_qdata_log = function(self, operation, kwargs)
					if _state.logging then
						_log(
							"[SDK] report_qdata_log(" .. tostring(operation) .. ") kwargs=" .. Utils.dump_value(kwargs)
						)
					end
				end,
